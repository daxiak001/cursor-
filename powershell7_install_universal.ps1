# Universal PowerShell 7 Installation Script
...
Write-Host "    Universal PowerShell 7 Installation Script" -ForegroundColor Green
Write-Host "=====================================================" -ForegroundColor Green
Write-Host ""
...
if (Test-Path $ps7Path) {
    try {
        ...
        Write-Host "SUCCESS: PowerShell 7 is already installed!" -ForegroundColor Green
        ...
        Write-Host "INFO: PowerShell 7 files found but may be corrupted, reinstalling..." -ForegroundColor Yellow
    }
}
...
if ($useWinget) {
    ...
    try {
        ...
        Write-Host "ERROR: winget installation failed, trying manual download..." -ForegroundColor Red
        $useWinget = $false
    }
}
...
if (-not $useWinget) {
    ...
    try {
        ...
        Write-Host "SUCCESS: Download completed: $ps7File" -ForegroundColor Green
        ...
        Write-Host "SUCCESS: PowerShell 7 installation completed!" -ForegroundColor Green
...
try {
    if (Test-Path $ps7Path) {
        $ps7Version = & $ps7Path -Command '$PSVersionTable.PSVersion.ToString()' 2>$null
        Write-Host "SUCCESS: PowerShell 7 installation verified!" -ForegroundColor Green
        Write-Host "Version: $ps7Version" -ForegroundColor Cyan
        Write-Host "Location: $ps7Path" -ForegroundColor Gray
        Write-Host ""
    ...
    Write-Host "ERROR: PowerShell 7 verification failed" -ForegroundColor Red
